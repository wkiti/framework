...
└── MyNewApp/                  <-- Generated Project Instance (via CLI)
    ...
    ├── public/                <-- Generated Assets (CSS, JS, Images)
...
- Networking & Security:
  ...
  * OpenSSL (-lssl -lcrypto -lcrypt32) [HTTPS TLS 1.3 Encryption & SHA-256]
  * nlohmann/json (Header-only, included in include/nlohmann/json.hpp)
- Database Drivers:
  * SQLite3 (Embedded default, included in src/sqlite3.o)
  * PostgreSQL (Native C Driver libpq: -lpq)
  * MySQL / MariaDB (Native C Driver libmariadb: -lmariadb)

Package Installation Commands:
  • Windows MSYS2 UCRT64:
    pacman -S mingw-w64-ucrt-x86_64-gcc mingw-w64-ucrt-x86_64-openssl mingw-w64-ucrt-x86_64-postgresql mingw-w64-ucrt-x86_64-libmariadbclient

  • Ubuntu / Debian Linux:
    sudo apt update && sudo apt install -y build-essential g++ cmake libssl-dev libsqlite3-dev libpq-dev libmariadb-dev

  • macOS Homebrew:
    brew install cmake openssl sqlite postgresql mariadb-connector-c
...
2. Compile Wkiti Framework (Standard Default Build with SQLite):
...
3. Compile with PostgreSQL and MySQL Drivers Enabled:
   g++ -O3 -std=c++17 -I./include -I"C:/msys64/ucrt64/include" `
   src/main.cpp src/Server.cpp src/Database.cpp src/Model.cpp src/Request.cpp src/Response.cpp src/Security.cpp src/sqlite3.o `
   -o wkiti_app.exe `
   -L"C:/msys64/ucrt64/lib" -lws2_32 -lssl -lcrypto -lcrypt32 -lpq -lmariadb

4. Launch Wkiti Server:
   .\wkiti_app.exe
...
Wkiti includes an automated CLI generator tool to scaffold standalone 
projects with C++ backend code, HTML templates, CSS stylesheets, and JS scripts.

1. Compile the CLI tool:
   g++ -O3 -std=c++17 -I./include src/cli.cpp -o wkiti-cli.exe
...
sources, HTML templates, CSS stylesheets, script.js, and main.cpp.
...
--- B. TEMPLATES, CSS & JAVASCRIPT ---
// Serve static CSS, JS, and image files from 'public/'
...
--- C. MULTI-DATABASE ENGINE & ORM (SQLITE, POSTGRESQL, MYSQL) ---

Wkiti supports multiple database engines through a unified ORM API and 
automatic URI driver detection.

1. SQLite Connection (Embedded File-based Default):
   Wkiti::Database db("sqlite://wkiti_app.db");

2. PostgreSQL Connection (Enterprise libpq Driver):
   Wkiti::Database db("postgresql://postgres:secret@localhost:5432/wkiti_db");

3. MySQL / MariaDB Connection (Scalable C Client Driver):
   Wkiti::Database db("mysql://root:secret@localhost:3306/wkiti_db");

// Automatic Placeholder Normalization:
// PostgreSQL uses $1, $2 parameter placeholders instead of ?. 
// Wkiti automatically translates ? to $1, $2 for PostgreSQL, keeping 
// your C++ queries 100% portable across SQLite, Postgres, & MySQL!

// Link Active DB Engine to ORM
...
u.save(); // Uses Prepared Statements & Hashes Password with OpenSSL SHA-256
...
Wkiti includes a dedicated test runner verifying HTTP parsing, SHA-256 
hashing, JSON formatting, and SQLite/PostgreSQL/MySQL driver detection.

1. Compile the Test Suite:
   g++ -O3 -std=c++17 -I./include -I"C:/msys64/ucrt64/include" `
   tests/test_runner.cpp src/Database.cpp src/Request.cpp src/Response.cpp src/Security.cpp src/sqlite3.o `
...
Wkiti::Server::on_error()        void on_error(int code, Handler h)           Register custom 404/500 error page
Wkiti::Server::use_https()       void use_https(cert, key)                    Enable TLS 1.3 HTTPS Encryption
...
Wkiti::Database::Database()      Database(string conn_uri)                    Detect & Connect SQLite/Postgres/MySQL
Wkiti::Database::get_type()      DatabaseType get_type()                      Returns active driver enum
